...
class Model(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        output = self.decoder(batch["decoder_input"], encoder_output, batch["mask"])
        return output
        ...
        for idx, batch in enumerate(dataloader):
            ...
            target = (
                torch.nn.functional.one_hot(
                    batch["decoder_output"], dict_vocab_size["english"]
                )
                .float()
                .to(device)
            )
            ...
            expected_text = np.array(dataset.english_vocabrary.lookup_tokens(
                batch["decoder_output"][0, :].tolist()
            ))
            output_token = torch.nn.functional.softmax(output[0, :], dim=-1).argmax(
                dim=-1
            )  # greedy
            output_text = np.array(dataset.english_vocabrary.lookup_tokens(output_token.tolist()))
            
            for token in ['<pad>', '<eos>']:
                expected_text = expected_text[expected_text!=token]
                output_text   = output_text[output_text!=token]
                
            output_text = output_text.tolist()
            expected_text = expected_text.tolist()
            
            print(
                f"epoch: {epoch} iteration: {idx} loss: {train_loss/(idx+1):.5f} expected_text: {' '.join(expected_text)} output_text: {' '.join(output_text)}"
            )
